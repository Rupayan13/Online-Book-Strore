...
$select = $con->query("SELECT * FROM categories");
$select->execute();
  ...
  if(empty($_POST['name']) OR empty($_POST['description']) OR empty($_POST['price']))
    {
      ...
      $insert = $con->prepare("INSERT INTO products (name, description, price, category_id, image, file) VALUES (:name, :description, :price, :category_id, :image, :file)");
      $insert->execute([
        ":name"=>$name,
